...
app.post("/login", async (req, res) => {
 ...
 // obtener todos los enlaces publicados hoy y en días anteriores
  app.get("/links", authMiddleware, async (req, res) => {
    ...
    const query = { publishedDate: { $lte: today, $gte: yesterday } };
    try {
      const result = await linkModel.find(query);
...
app.delete("/links/:id", authMiddleware, async (req, res) => {
  ...
  // Votar por un link
app.post("/links/:id/vote", async (req, res) => {
  const linkId = req.params.id;
  const user = req.user;
  if (!user) {
    res.status(401).json({ message: "You must be logged in to do that" });
    ...
    try {
      const result = await voteModel.create(linkId, user.id);
      res.status(200).json(result);
    } catch (error) {
      console.error(error);
